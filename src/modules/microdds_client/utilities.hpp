...
static bool create_data_writer(uxrSession *session, uxrStreamId create_stream_id, uxrObjectId participant_id,
			       ...
			       uxrObjectId &datawriter_id)
{
	...
	uint16_t topic_req = uxr_buffer_create_topic_bin(session, create_stream_id, topic_id, participant_id, topic_name,
			     type_name, UXR_REPLACE);


	// publisher
	uxrObjectId publisher_id = uxr_object_id(topic_id.id, UXR_PUBLISHER_ID);
	uint16_t publisher_req = uxr_buffer_create_publisher_bin(session, create_stream_id, publisher_id, participant_id,
				 UXR_REPLACE);
	...
	uint16_t datawriter_req = uxr_buffer_create_datawriter_bin(session, create_stream_id, datawriter_id, publisher_id,
...
static bool create_data_reader(uxrSession *session, uxrStreamId stream_id, uxrStreamId input_stream,
			       ...
			       const char *type_name)
{
	...
	uxrObjectId topic_id = uxr_object_id(id, UXR_TOPIC_ID);
	uint16_t topic_req = uxr_buffer_create_topic_bin(session, stream_id, topic_id, participant_id, topic_name, type_name,
			     UXR_REPLACE);
	...
	uint16_t datareader_req = uxr_buffer_create_datareader_bin(session, stream_id, datareader_id, subscriber_id, topic_id,
				  qos, UXR_REPLACE);
